...
def produce_message():
    """Envoie un message de test à Kafka que Spark Streaming pourra consommer"""
    producer = KafkaProducer(
        bootstrap_servers=KAFKA_BROKER,
        value_serializer=lambda v: json.dumps(v).encode("utf-8")
    )

    # Générer des données structurées pour Spark Streaming
    # Exemple d'un message avec des données temporelles, type d'événement et valeur
    message = {
        "timestamp": time.time(),  # Timestamp pour simuler un événement temporel
        "event_type": "data_event",  # Type d'événement
        "value": {"data_point": 42, "description": "Exemple de donnée"}
    }
